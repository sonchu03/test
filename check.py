...
def predict_score(name):
    ...
    result = {
        'selected_movie': str(new_movie.Title.values[0]),
        'similar_movies': [],
        'predicted_score': 0,
        'actual_score': float(new_movie['Vote Average'].iloc[0]),
    }
    K = 10
    avgRating = 0
    ...
    for neighbor in neighbors:
        ...
        result['similar_movies'].append({
            'title': movies.iloc[neighbor[0]][0],
            'genres': str(movies.iloc[neighbor[0]][1]).strip('[]').replace(' ',''),
            'rating': float(movies.iloc[neighbor[0]][2]),
        })
        
    avgRating = avgRating / K
    result['predicted_score'] = round(avgRating,3)
